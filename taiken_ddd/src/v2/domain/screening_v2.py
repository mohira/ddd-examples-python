...
class ScreeningV2:
    ...
    status: ScreeningStatusV2 = field(init=False)
    ...
    interviews: List[InterviewV2] = field(init=False)

    @classmethod
    def start_from_pre_interview(cls, applicant_email_address: str) -> ScreeningV2:
        ...
        object.__setattr__(screening, 'status', ScreeningStatusV2.NotApplied)  # ① 面談からの場合はステータス「未応募」で登録
    ...
    @classmethod
    def apply(cls, applicant_email_address: str) -> ScreeningV2:
        ...
        object.__setattr__(screening, 'screening_id', str(uuid.uuid4()))
        object.__setattr__(screening, 'status', ScreeningStatusV2.Interview)  # ③ 面接なので初期ステータス「選考中」で登録
    ...
    def add_next_interview(self, interview_date: datetime.date) -> None:
        ...
        if self.status != ScreeningStatusV2.Interview:
            raise ApplicationException('不正な操作です。選考中でないときに次の面接を設定することはできません')

        # ② 面接次数は1からインクリメントされる
        next_interview_number = len(self.interviews) + 1
        next_interview = InterviewV2(interview_date, next_interview_number)
