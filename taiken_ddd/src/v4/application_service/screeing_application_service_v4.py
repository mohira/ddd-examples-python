...
from taiken_ddd.src.v4.domain.screening_repository import ScreeningRepository
...
class ScreeningApplicationServiceV4:
    ...
    def start_from_pre_interview(self, applicant_email_address: str) -> None:
        """面談から新規候補者を登録する"""
        screening = ScreeningV4.start_from_pre_interview(applicant_email_address)

        self.screening_repository.insert(screening)

    def apply(self, applicant_email_address: str) -> None:
        """新規応募者を登録する"""
        screening = ScreeningV4.apply(applicant_email_address)
